"""Agent plan wire validation (burn epochs vs planning reference)."""
...
def test_burn_epoch_future_fails_when_burn_before_reference() -> None:
    ref = datetime(2030, 1, 1, 12, 0, 0, tzinfo=UTC)
    row = burn_epoch_future_validation_row(
    ...
    assert row["check_id"] == "BURN_EPOCH_NOT_IN_PAST"


def test_plan_dict_validation_passed_requires_future_burns_when_validation_empty() -> None:
    ref = datetime(2030, 6, 1, 0, 0, 0, tzinfo=UTC)
    plan = {
        "maneuvers": [{"epoch_utc": "2030-05-01T00:00:00Z"}],
        "validation": [],
    }
    assert plan_dict_validation_passed(plan, not_before=ref) is False
...
def test_margin_allows_one_second_before_reference() -> None:
    ref = datetime(2030, 1, 1, 12, 0, 0, tzinfo=UTC)
